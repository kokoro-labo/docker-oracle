...
RUN cd /tmp && \
    ...
    rm oracle-database-ee-21c-1.0-1.ol8.x86_64.rpm

# configure oracle 21c
...
RUN cp -a /etc/sysconfig/oracledb_ORCLCDB-21c.conf /etc/sysconfig/oracledb_CDB-21c.conf && \
    sed -e s/ORCLCDB/CDB/g -e s/ORCLPDB1/PDB1/g /etc/init.d/oracledb_ORCLCDB-21c > /etc/init.d/oracledb_CDB-21c && \
    chmod 755 /etc/init.d/oracledb_CDB-21c && \
    env ORACLE_DOCKER_INSTALL=true && \
    /etc/init.d/oracledb_CDB-21c configure

# copy setup files
COPY --chown=oracle:oinstall setup_21c/ /tmp/
